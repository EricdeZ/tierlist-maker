...
function VaultInner() {
    ...
    const dismissDiscord = () => {
        ...
        const count = parseInt(localStorage.getItem('discord-join-dismiss-count') || '0', 10) + 1
    ...
    return (
        ...
        {showDiscordPromo && !passionLoading && !inDiscord && user && !showTradematchPromo && !showPromoGift && !promoGiftResult && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm" onClick={dismissDiscord}>
                <div
                    className="relative w-full max-w-sm mx-4 rounded-2xl overflow-hidden p-6 text-center"
                    style={{ background: 'var(--cd-surface)', border: '1px solid rgba(88,101,242,0.3)', boxShadow: '0 0 40px rgba(88,101,242,0.15)', animation: 'cd-fade-in 0.3s ease-out' }}
                    onClick={e => e.stopPropagation()}
                >
                    <svg className="w-10 h-10 mx-auto mb-3 text-[#5865F2]" viewBox="0 0 24 24" fill="currentColor">
                        <path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028c.462-.63.874-1.295 1.226-1.994a.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03z" />
                    </svg>
                    <h2 className="text-lg font-bold cd-head tracking-wider mb-2" style={{ color: '#5865F2' }}>
                        Join Our Discord
                    </h2>
                    <p className="text-sm mb-4" style={{ color: 'var(--cd-text-dim)' }}>
                        Connect with the smitecomp.com team and community:
                    </p>
                    <ul className="text-sm text-left mb-5 space-y-2 px-2" style={{ color: 'var(--cd-text-dim)' }}>
                        <li className="flex items-start gap-2">
                            <span className="text-[#5865F2] mt-0.5 shrink-0">&#x2022;</span>
                            Share feedback and ideas directly with the team
                        </li>
                        <li className="flex items-start gap-2">
                            <span className="text-[#5865F2] mt-0.5 shrink-0">&#x2022;</span>
                            Read patch notes as soon as they drop
                        </li>
                        <li className="flex items-start gap-2">
                            <span className="text-[#5865F2] mt-0.5 shrink-0">&#x2022;</span>
                            Get notified when the vending machine restocks
                        </li>
                        <li className="flex items-start gap-2">
                            <span className="text-[#5865F2] mt-0.5 shrink-0">&#x2022;</span>
                            Join exclusive pack code giveaways
                        </li>
                    </ul>
                    <div className="flex flex-col gap-2">
                        <a
                            href="https://discord.gg/Z6WfPckRUJ"
                            target="_blank"
                            rel="noopener noreferrer"
                            onClick={dismissDiscord}
                            className="w-full py-2.5 rounded-xl text-sm font-bold cd-head tracking-wider text-white transition-all active:scale-95 cursor-pointer text-center block"
