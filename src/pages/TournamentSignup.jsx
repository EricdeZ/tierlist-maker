...
export default function TournamentSignup() {
    ...
    useEffect(() => {
        if (gameState !== 'ended' || coresAwarded !== null || !tournament) return
    ...
    }, [gameState, coresAwarded, tournament, eggsCollected])
    ...
    const gameActive = isEaster && user && playsRemaining !== null && playsRemaining > 0
        && tournament.status !== 'completed'

    // Dynamically import easter.css only when needed
    if (isEaster) import('./tournament/easter.css')

    return (
        <div className={isEaster ? 'easter-bg' : ''}>
            {/* Easter canvas — always renders for visual ambiance when Easter */}
            {isEaster && (
                <Suspense fallback={null}>
                    <EasterCanvas
                        formRef={formRef}
                        onEggCollected={handleEggCollected}
                        enabled={gameActive && gameState !== 'ended'}
                    />
                </Suspense>
            )}

            {/* Easter HUD — only when game is available */}
            {gameActive && (
                <Suspense fallback={null}>
                    <EasterHUD
            ...
            <div className="max-w-3xl mx-auto py-8 px-4 relative z-2">
                ...
                {/* Standard header — hidden for Easter (hedge banner replaces it) */}
                {!isEaster && (
                    <div className="mb-8">
                ...
                {tournament.description && (
                    <div className={`mb-8 p-6 rounded-xl ${isEaster ? 'easter-form-card' : 'bg-white/5 border border-white/10'}`}>
                        <div className="text-(--color-text) whitespace-pre-wrap leading-relaxed">{tournament.description}</div>
                    </div>
                )}

                {/* Dates */}
                <div ref={formRef} className={`mb-8 p-6 rounded-xl ${isEaster ? 'easter-form-card' : 'bg-white/5 border border-white/10'}`}>
                ...
                {/* Signup Section */}
                {!user ? (
                    <div className={`p-6 rounded-xl text-center ${isEaster ? 'easter-form-card' : 'bg-white/5 border border-white/10'}`}>
                        <p className="text-(--color-text-secondary) mb-4">You must be logged in to sign up for this tournament.</p>
                        <button onClick={login} className={`px-6 py-2.5 rounded-lg font-semibold transition-opacity hover:opacity-90 ${isEaster ? 'easter-submit-btn' : 'bg-(--color-accent) text-black'}`}>
                            Log in with Discord
                        </button>
                    </div>
                ) : signup || submitSuccess ? (
                    <div className={`p-6 rounded-xl ${isEaster ? 'easter-form-card' : 'bg-white/5 border border-white/10'}`}>
                    ...
                    <form onSubmit={handleSubmit} className={`p-6 rounded-xl space-y-6 ${isEaster ? 'easter-form-card' : 'bg-white/5 border border-white/10'}`}>
