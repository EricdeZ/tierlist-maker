...
import { Plus, Pencil, Trash2, ChevronDown, ChevronRight, Swords, Tag, Settings2, X, Link2, FolderTree, ImagePlus, Copy, GripVertical, Layers, Save } from 'lucide-react'
...
export default function CodexGods() {
    ...
    const saveFieldAsType = async (field) => {
        ...
        try {
            ...
            await codexService.updateGodField({ id: field.id, group_type_id: result.groupType.id })
            setFieldPopup(null)
            await fetchData()
        } catch (err) {
            alert('Error: ' + err.message)
    ...
    const linkFieldToType = async (fieldId) => {
        ...
        setSaving(true)
        try {
            await codexService.updateGodField({ id: fieldId, group_type_id: typeId })
            setFieldPopup(null)
            await fetchData()
        } catch (err) {
            alert('Error: ' + err.message)
