...
import { Plus, Pencil, Trash2, ChevronDown, ChevronRight, Swords, X, Link2, FolderTree, ImagePlus, Copy } from 'lucide-react'
...
export default function CodexGods() {
    ...
    // Section collapse state (god-specific)
    ...
    const crud = useCodexCrud({
        fieldApi: { create: codexService.createGodField, update: codexService.updateGodField, delete: codexService.deleteGodField },
        tagApi: { create: codexService.createGodTag, update: codexService.updateGodTag, delete: codexService.deleteGodTag },
        groupTypeApi: { create: codexService.createGroupType, update: codexService.updateGroupType, delete: codexService.deleteGroupType },
        fieldUpdateApi: codexService.updateGodField,
        fetchData,
    })
    ...
    return (
        <div className="max-w-5xl mx-auto px-4 py-8">
            ...
            <CodexFieldsSection
                ...
                saveField={crud.saveField} deleteField={(f) => crud.deleteField(f, 'god')}
            ...
            <CodexTagsSection
                ...
                saveTag={crud.saveTag} deleteTag={(t) => crud.deleteTag(t, 'gods')}
                saving={crud.saving}
                entityLabel="gods"
            />
                        ...
                        {categoryForm ? (
                            <div className="border border-white/10 rounded-lg p-4 bg-black/10">
                                ...
                                <div className="flex items-center gap-3 mt-4">
                                    <button onClick={saveCategory} disabled={crud.saving || !categoryForm.name?.trim()}
                                        ...
                                        {crud.saving ? 'Saving...' : (categoryForm.id ? 'Update' : 'Create')}
                                    </button>
            ...
            <div>
                ...
                {godForm && (
                    <div className="bg-(--color-secondary) border border-white/10 rounded-xl p-5 mb-4">
                        ...
                        {fields.length > 0 && (
                            <div className="mb-4">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                                    {fields.map(field => (
                                        <div key={field.id} className={field.field_type === 'group' ? 'sm:col-span-2 lg:col-span-3' : (field.field_type === 'text' && crud.isLongText(godForm.field_values[field.slug])) ? 'sm:col-span-2 lg:col-span-3' : ''}>
                                            ...
                                            {field.field_type === 'group' && field.options?.sub_fields ? (
                                                ...
                                                <div className={field.field_type === 'percentage' ? 'relative' : ''}>
                                                    {field.field_type === 'text' && crud.isLongText(godForm.field_values[field.slug]) ? (
                                                        <textarea
                                                            value={godForm.field_values[field.slug] ?? ''}
                                                            onChange={e => { crud.trackFocus(field.slug, e.target); setGodFieldValue(field.slug, e.target.value); crud.autoResize(e.target) }}
                                                        ...
                                                        <input
                                                            ...
                                                            value={godForm.field_values[field.slug] ?? ''}
                                                            onChange={e => { if (field.field_type === 'text') crud.trackFocus(field.slug, e.target); setGodFieldValue(field.slug, e.target.value) }}
                                                            ref={crud.fieldRef(field.slug)}
                        ...
                        {/* Save / Cancel */}
                        <div className="flex items-center gap-3">
                            <button onClick={saveGod} disabled={crud.saving || !godForm.name?.trim()}
                                className="px-4 py-2 rounded-lg bg-(--color-accent) hover:bg-(--color-accent)/80 text-white text-sm font-semibold transition-colors cursor-pointer disabled:opacity-50">
                                {crud.saving ? 'Saving...' : (godForm.id ? 'Update' : 'Create')}
                            </button>
